...
- The goal: Understand the ratio of fishstock biomass inside and outside of a sanctuary no-take zone. 
...
population_illegal <- declare_population(
  fishers = add_level(N=75, 
     illegal_fishing=sample(c(rep(0,50),rep(1,25))),
...
```
Here, we see out of 75 fishers in the region, we see 25 counts of fishing illegally, 13/15 are reported by the *guards*  

I'll look at how we would view a response bias


## Declaring response bias
I first wrote this out ot try and represent the fishers who were "seen" by enforcement officers. However, 
```{r}
# reporting <- declare_assignment(blocks=illegal_fishing,
...
store <- rep(NA, sims)
for (i in 1:sims){
  store[i] <- mean(pop_illegal[sample(1:500,sam.n),] %>%
...
population_2 <- declare_population(
  fishers = add_level(N=100,
     ...
     educated=correlate(given = educated, rho = 0.5,
                         draw_binary, prob = 0.5)

))
pop_2 <- population_2() #PH: we're using Declare Design here to create a population of 100 fishers, half of which are educated (answer the question correctly). Then, we're telling R to use whether or not fishers are educated to determine whether they avoid the sanctuary. We assume that fisher knowledge about the issue of stock decreases is correlated with the preventative measure of avoiding the sanctuary.

kable(table(pop_2$educated,pop_2$avoid_sanctuary)) %>% 
...
reporting <- declare_assignment(blocks=educated,
                  assignment_variable = "R", 
                  block_prob=c(0.7,0.9)) 
pop_reporting <- reporting(pop_2)
# check it out
kable(pop_reporting[1:10,])
table(pop_reporting$educated, pop_reporting$R)
```
